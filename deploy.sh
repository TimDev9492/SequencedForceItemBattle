...
destination="$docker_dir/data/servers/challenges/plugins/"
screen_session_name="minecraft-server-challenges"
...
ssh -i "$key_file" "$host" "cd \"$docker_dir\" && docker compose exec server-mgr screen -S \"$screen_session_name\" -p 0 -X stuff \"plugman reload SequencedForceItemBattle\"\`echo -ne '\015'\`"
